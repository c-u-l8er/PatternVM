defmodule PatternVM do
  ...
  def init(_) do
    ...
    # Start observer for visualization
    {:ok, observer_pid} =
      PatternVM.Supervisor.start_child(PatternVM.Observer, topics: ["pattern_logs"])
  ...
  def notify_observers(topic, data) do
    Phoenix.PubSub.broadcast(PatternVM.PubSub, topic, {:update, Map.put(data, :topic, topic)})
  end
  ...
  def handle_call({:register_pattern, pattern_module, config}, _from, state) do
    pattern_name = pattern_module.pattern_name()
